...
def _mock_patchAndRecreateGlobusUDT(srpmFile, patchFile, mockConfigFile=None, mockRoot=None):
  """ Patch and recreate an SRPM """

  tmpDir = diracoslib._extractSRPM(srpmFile)
  tarFn = os.path.join(tmpDir, "globus_xio_udt_driver-1.20.tar.gz")
...
def execute(**kwargs):
  ...
  logging.info("Executing glite-ce-cream-cli routine")
  ...
  if patchFile:
    newSRPM = _mock_patchAndRecreateGlobusUDT(SRPMFile, patchFile, mockConfig,mockRoot = mockRoot)
